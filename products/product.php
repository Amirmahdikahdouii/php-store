...
$sql = "SELECT * FROM `product_comment` WHERE product_id='$productID' LIMIT 5 OFFSET 0";
...
<body>
    ...
    <section class="main-section">
        <div class="product-info-container">
            ...
            <div class="coffe-type">
                ...
                <div class="cup-sizes center">
                    <button class="cup-size center cup-size-active" onclick="cupSelectColorize(0, <?= number_format($Product["price"], 2, ".") ?>)" title="150 cc">
                        <i class="bi bi-cup-fill" style="font-size: 20px"></i>
                    </button>
                    <button class="cup-size center" onclick="cupSelectColorize(1, <?= number_format($Product["price"], 2, ".") ?>)" title="250 cc">
                        <i class="bi bi-cup-fill" style="font-size: 30px"></i>
                    </button>
                    <button class="cup-size center" onclick="cupSelectColorize(2, <?= number_format($Product["price"], 2, ".") ?>)" title="350 cc">
                ...
                <?php
                $sql = "SELECT COUNT(*) FROM product_info_table WHERE product_id='$productID'";
                $productInfoTable = mysqli_query($db_connection, $sql);
                $productInfoTable = mysqli_fetch_assoc($productInfoTable);
                $productInfoTable = intval($productInfoTable['COUNT(*)']);
                if ($productInfoTable > 0) {
                ?>
        ...
        <div class="product-image-part">
            <div class="center product-image-container">
                <img src="../media/product-image/<?= $Product["image"] ?>" class="product-image" id="main-product-image" alt="<?= $Product["name"] ?>">
            </div>
            <?php
                ...
                <div class="more-pictures-container center">
                    ...
                    ?>
                        <div class="more-picture-item center">
                            <img src="../media/product-image/<?= $ProductGalleryImages[$i]["image"] ?>" alt="<?= $Product["name"] ?>" class="more-picture-item-image">
                        </div>
                    <?php
                    }
                    ?>
            ...
            ?>
                <div class="carousel-product-card <?php if ($i <= 4) echo "carousel-product-card-active"; ?>">
                    <div class="carousel-product-card-image-container">
                        <img src="../media/product-image/<?= $relatedProduct["image"] ?>" class="center carousel-product-card-image" alt="<?= $relatedProduct['name'] ?>" />
                        <div class="carousel-product-card-image-dark-theme">
                            <a href="./product.php?id=<?= $relatedProduct['id'] ?>" class="carousel-add-to-cart-button"><i class="bi bi-bag-plus"></i></a>
                        </div>
                    </div>
    ...
    if ($productInfoTable > 0) {
        ...
        $sql = "SELECT * FROM product_info_table WHERE product_id='$productID'";
        $productInfoRows = mysqli_query($db_connection, $sql);
        $productInfoRows = mysqli_fetch_all($productInfoRows, MYSQLI_ASSOC);
    ?>
        <section class="product-more-detail-section">
            <div class="product-more-detail-container">
                ...
                <div class="product-more-detail-table">
                    <?php
                    for ($productRowIndex = 0; $productRowIndex < count($productInfoRows); $productRowIndex++) {
                    ?>
                        <div class="product-more-detail-table-row">
                            <div class="product-more-detail-table-column">
                                <span class="product-more-detail-table-column-span"><?= $productInfoRows[$productRowIndex]['property'] ?></span>
                            </div>
                            <div class="product-more-detail-table-column">
                                <span class="product-more-detail-table-column-span" id="detail-weight-size"><?= $productInfoRows[$productRowIndex]['value'] ?></span>
                            </div>
                        </div>
                    <?php
                    }
                    if (count($productInfoRows) > 4) {
                    ?>

                        <span class="product-more-detail-table-read-more" id="product-more-detail-table-read-more-button">Read
                            More</span>
                        <span class="product-more-detail-table-read-more" id="product-more-detail-table-read-less-button">Close</span>
                    <?php
                    }
                    ?>
                </div>
        ...
        <div class="product-comments-section-container">
            <div class="past-comments-container">
                ...
                ?>
                    <div class="past-comment-item">
                        ...
                        <?php
                        // Show This Box only when user is login!
                        ...
                        ?>
                            <div class="comment-like-dislike-container">
                                ...
                                $sql = "SELECT * FROM product_comment_like WHERE comment_id='$commentID' AND user_id='$user_id'";
                                ...
                                ?>
                                    <i class="bi bi-heart-fill" commentID='<?= $comment['id'] ?>'></i>
                                <?php
                                } else {
                                ?>
                                    <i class="bi bi-heart" commentID='<?= $comment['id'] ?>'></i>
                                <?php
                                }
                                ?>
                            </div>
            ...
            <div class="new-comments-container">
                ...
                <form class="new-comment-form">
                    ...
                    <textarea class="new-comment-text-area" id="new-comment-message" name="new-comment" placeholder="MESSAGE ..."></textarea>
    ...
    if ($productInfoTable > 0) {
    ?>
        <script src="/php-store/static/js/products/products-tableInfo.js"></script>
    <?php
    }
    ?>
