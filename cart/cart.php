...
if (count($result) > 0) {
    ...
    <section class="main-container">
                ...
                <div class="cart-list-item">
                    <div class="cart-list-item-image-count-container">
                        <div class="cart-list-item-image-container center">
                            <img src="/php-store/media/product-image/<?= $product['image']; ?>" alt="Coffee"
                                 class="cart-list-item-image">
                        </div>
                        <div class="cart-list-item-mobile-product-title-container">
                            <a class="cart-list-item-title"
                               href="/php-store/products/product.php?id=<?= $product["id"] ?>">
                                <?= $product['name'] ?>
                            </a>
                        </div>
                        <?php
                        if ($userLogin) {
                            ...
                            <div class="center cart-list-item-count-container">
                                ...
                                <button class="center cart-list-item-mines-button <?php if ($countOfProduct === 1) echo 'cart-list-item-cart-info-plus-count-button-deactive' ?>"
                                        productID="<?= $product["id"] ?>">
                                    <i class="bi bi-dash-lg"></i>
                                </button>
                                <span class="center cart-list-item-count"><?= $countOfProduct; ?></span>
                                <button class="center cart-list-item-plus-button <?php if ($countOfProduct === 10) echo 'cart-list-item-cart-info-plus-count-button-deactive' ?>"
                                        productID="<?= $product["id"] ?>">
        ...
        <aside class="aside-checkout-container">
            ...
            if ($userLogin) {
                ?>
                <button class="continue-shopping-button" id="confirm-shoping-button" userID="<?= $user_id ?>"
                        cartID="<?= $cart_id ?>">Continue
                </button>
                <?php
            } else {
                ?>
                <a style="text-align: center" href="/php-store/Accounts/login.php" class="continue-shopping-button">Login
                    to Continue</a>
                <?php
            }
            ?>
